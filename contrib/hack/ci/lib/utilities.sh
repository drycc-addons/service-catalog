...
kind::create_cluster() {
    ...
    kind create cluster --name=${KIND_CLUSTER_NAME}-${CI_COMMIT_SHA:-"latest"} --image="kindest/node:${KUBERNETES_VERSION}" --wait=5m
}

kind::delete_cluster() {
    kind delete cluster --name=${KIND_CLUSTER_NAME}-${CI_COMMIT_SHA:-"latest"}
}
