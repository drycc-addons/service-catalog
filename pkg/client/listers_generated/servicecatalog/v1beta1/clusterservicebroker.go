...
type ClusterServiceBrokerLister interface {
	...
	List(selector labels.Selector) (ret []*servicecatalogv1beta1.ClusterServiceBroker, err error)
	// Get retrieves the ClusterServiceBroker from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*servicecatalogv1beta1.ClusterServiceBroker, error)
...
type clusterServiceBrokerLister struct {
	listers.ResourceIndexer[*servicecatalogv1beta1.ClusterServiceBroker]
}
...
func NewClusterServiceBrokerLister(indexer cache.Indexer) ClusterServiceBrokerLister {
	return &clusterServiceBrokerLister{listers.New[*servicecatalogv1beta1.ClusterServiceBroker](indexer, servicecatalogv1beta1.Resource("clusterservicebroker"))}
}
