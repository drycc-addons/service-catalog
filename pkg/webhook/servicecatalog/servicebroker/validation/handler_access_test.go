...
import (
	...
	"github.com/drycc-addons/service-catalog/pkg/webhook/servicecatalog/servicebroker/validation"
...
func (m *fakedClient) Create(ctx context.Context, obj client.Object, opts ...client.CreateOption) error {
...
func TestSpecValidationHandlerAccessToBrokerDenied(t *testing.T) {
	...
	for desc, test := range tests {
		t.Run(desc, func(t *testing.T) {
			...
			fakeClient := fake.NewClientBuilder().WithScheme(sch).WithObjects(&sc.ServiceBroker{}).Build()
