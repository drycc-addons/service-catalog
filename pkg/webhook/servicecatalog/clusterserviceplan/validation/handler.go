...
type Validator interface {
	Validate(context.Context, admission.Request, *sc.ClusterServicePlan, *webhookutil.TracedLogger) *webhookutil.WebhookError
}
...
func (h *SpecValidationHandler) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	csp := &sc.ClusterServicePlan{}
	if err := webhookutil.MatchKinds(csp, req.Kind); err != nil {
		traced.Errorf("Error matching kinds: %v", err)
		return admission.Errored(http.StatusBadRequest, err)
	}

	if err := h.decoder.Decode(req, csp); err != nil {
		traced.Errorf("Could not decode request object: %v", err)
		return admission.Errored(http.StatusBadRequest, err)
	}

	traced.Infof("start validation process for %s: %s/%s", csp.Kind, csp.Namespace, csp.Name)
	...
	case admissionTypes.Create:
		for _, v := range h.CreateValidators {
			err = v.Validate(ctx, req, csp, traced)
			if err != nil {
				break
			}
		}
	...
	return admission.Allowed("ClusterServicePlan validation successful")
}
